...
@admin_router.post("/updateShippingStatus/{order_id}", response_class=HTMLResponse)
async def update_shipping_status(
    ...
    logging.info(f"Received update request for Order ID: {order_id}")
    logging.info(f"Shipping Status received: {shipping_status}")

    order = db.query(Order).filter(Order.omno == order_id).first()
    if not order:
        logging.error(f"Order with ID {order_id} not found.")
        raise HTTPException(status_code=404, detail="Order not found.")

    logging.info(f"Current shipping status: {order.shipping_status}")
    order.shipping_status = shipping_status
    db.commit()
    logging.info(f"Updated shipping status to: {order.shipping_status}")

    return RedirectResponse('/admin/mgorder', status_code=303)
