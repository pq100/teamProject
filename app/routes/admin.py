...
async def delete_product(prdno: int, db: Session = Depends(get_db)):
...
async def edit_product(request: Request, prdno: int, db: Session = Depends(get_db)):
    product = db.query(ProductModel).filter(ProductModel.prdno == prdno).first()
...
async def update_product(prdno: int, product: NewProduct = Depends(ProductService.get_product_data),
                         files: List[UploadFile] = File(...), db: Session = Depends(get_db)):
    try:
